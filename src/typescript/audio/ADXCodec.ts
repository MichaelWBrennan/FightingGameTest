...
export class ADXDecoder {
  private header: ADXHeader | null = null;
  private channelStates: ADXChannelState[] = [];
  ...
  decodeHeader(buffer: ArrayBuffer): boolean {
    ...
    if (view.getUint16(0, false) !== 0x8000) return false;
  ...
  decodeBlock(blockData: Uint8Array, channel: number): Int16Array {
    if (!this.header) throw new Error('Header not decoded');
