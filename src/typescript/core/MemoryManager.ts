...
export class MemoryManager {
  ...
  private defaultAlignment: number = 16;
   ...
   */
  createPool(name: string, baseAddress: number, size: number, alignment: number = 16): boolean {
  ...
  free(poolName: string, address: number): boolean {
    ...
    const blockIndex = pool.blocks.findIndex(block => 
      block.address === address && block.allocated
    );
   ...
   */
  private findFreeSpace(pool: MemoryPool, size: number): number | null {
   ...
   */
  getGlobalStats(): { total: number; used: number; free: number; pools: number } {
    return {
   ...
   */
  getAllocatedBlocks(): { poolName: string; block: MemoryBlock }[] {
    const blocks: { poolName: string; block: MemoryBlock }[] = [];
   ...
   */
  checkForLeaks(): { poolName: string; leaks: MemoryBlock[] }[] {
    const leaks: { poolName: string; leaks: MemoryBlock[] }[] = [];

    for (const [poolName, pool] of this.pools) {
      const poolLeaks = pool.blocks.filter(block => 
        block.allocated && !block.tag?.includes('persistent')
      );
