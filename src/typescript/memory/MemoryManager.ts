...
export class MemoryManager {
  ...
  private heaps: Map<string, MemoryHeap> = new Map();
  private memoryMap: ArrayBuffer;
  private memoryView: Uint8Array;
  private nextAddress: number = 0x10000000; // Mock starting address
   ...
   */
  private initializeDefaultHeaps(): void {
    // Main game heap
   ...
   */
  public createHeap(name: string, size: number): boolean {
    if (this.heaps.has(name)) {
      console.warn(`Heap '${name}' already exists`);
      return false;
    }

    const heap: MemoryHeap = {
      name,
   ...
   */
  public mmAlloc(size: number, heapName: string = 'MAIN'): number {
   ...
   */
  public mmAllocSub(size: number, heapName: string, userData: any): number {
   ...
   */
  private findFreeBlock(heap: MemoryHeap, size: number): number {
   ...
   */
  public mmFree(address: number, heapName: string = 'MAIN'): boolean {
    const heap = this.heaps.get(heapName);
    if (!heap) {
      console.error(`Heap '${heapName}' not found`);
      return false;
    ...
    const blockIndex = heap.blocks.findIndex(
      block => block.address === address && block.allocated
    );
   ...
   */
  public getHeapInfo(heapName: string): MemoryHeap | null {
   ...
   */
  public getMemoryStats(): { [heapName: string]: { total: number; used: number; free: number } } {
    const stats: { [heapName: string]: { total: number; used: number; free: number } } = {};
   ...
   */
  public compactHeap(heapName: string): void {
    const heap = this.heaps.get(heapName);
    if (!heap) {
      return;
    }
