...
export class StreamingOverlay {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    const btnCap = document.createElement('button'); btnCap.textContent = 'Capture'; btnCap.onclick = () => this.toggleCapture(); btnCap.style.marginRight = '6px';
    const btnRec = document.createElement('button'); btnRec.textContent = 'Record'; btnRec.onclick = () => this.toggleRecord(btnRec);
