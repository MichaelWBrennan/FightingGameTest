export class TournamentOverlay {
  ...
  constructor(private svc: any) {
    ...
    const label = document.createElement('div'); label.textContent = (i18n?.t?.('tournament') || 'Tournament'); label.style.fontWeight = 'bold';
    const input = document.createElement('textarea'); input.placeholder = (i18n?.t?.('players_placeholder') || 'player1\nplayer2\nplayer3'); input.style.width = '200px'; input.style.height = '80px';
    const gen = document.createElement('button'); gen.textContent = (i18n?.t?.('generate') || 'Generate'); gen.onclick = () => this.generate(input.value);
