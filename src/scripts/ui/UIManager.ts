...
class UIManager implements ISystem {
    ...
    // Cached top-level UI entities
    private uiRoot!: pc.Entity;
    private announcerText!: pc.Entity;
    private roundTimerText!: pc.Entity;
    private roundPipsP1!: pc.Entity;
    private roundPipsP2!: pc.Entity;
    ...
    private createHealthBar(playerId: 'player1' | 'player2', anchor: pc.Vec4, flip: boolean): void {
    ...
    private createMeterBar(playerId: 'player1' | 'player2', anchor: pc.Vec4, flip: boolean, segments: number): void {
        ...
        const segmentEntities: pc.Entity[] = [];
    ...
    private createComboDisplay(playerId: 'player1' | 'player2', anchor: pc.Vec4, color: pc.Color): void {
    ...
    private setupEventListeners(): void {
        ...
        this.app.on('gamestate:changed', (_old: string, next: string) => {
            if (next === 'BATTLE') {
                this.sequenceRoundStart();
            }
        });
        // Optional direct UI update hooks other systems can emit
        this.app.on('ui:health', (playerId: 'player1' | 'player2', current: number, max?: number) => this.setHealth(playerId, current, max));
        this.app.on('ui:meter', (playerId: 'player1' | 'player2', segmentsFilled: number) => this.setMeter(playerId, segmentsFilled));
        this.app.on('ui:timer', (seconds: number) => this.setTimer(seconds));
        this.app.on('ui:roundwins', (playerId: 'player1' | 'player2', wins: number) => this.setRoundWins(playerId, wins));
    }

    private onCombo(data: any): void {
        ...
        if (display.hitsText.element) display.hitsText.element.text = `${hits} HIT${hits > 1 ? 'S' : ''}!`;
        if (display.damageText.element) display.damageText.element.text = dmg > 0 ? `${Math.round(dmg)} dmg` : '';
        display.entity.enabled = true;
        display.visible = true;
        display.fadeTimer = 1.2; // seconds visible after last hit
    }

    private sequenceRoundStart(): void {
    ...
    private showAnnouncer(text: string): void {
        if (!this.announcerText.element) return;
        this.announcerText.element.text = text;
        this.announcerText.enabled = true;
    }

    private hideAnnouncer(): void {
        this.announcerText.enabled = false;
    }

    private setHealth(playerId: 'player1' | 'player2', current: number, max?: number): void {
        const hb = this.state.healthBars.get(playerId);
        if (!hb) return;
        if (typeof max === 'number' && max > 0) hb.maxHealth = max;
        ...
        if (hb.fill.element) {
            const anchor = hb.fill.element.anchor as pc.Vec4;
            hb.fill.element.anchor = new pc.Vec4(anchor.x, anchor.y, Math.max(0.001, ratio), anchor.w);
        }
    }

    private setMeter(playerId: 'player1' | 'player2', segmentsFilled: number): void {
    ...
    private setRoundWins(playerId: 'player1' | 'player2', wins: number): void {
    ...
    public destroy(): void {
        ...
        if (this.uiRoot && this.uiRoot.parent) this.uiRoot.destroy();
