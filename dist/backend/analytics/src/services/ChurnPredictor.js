...
export class ChurnPredictor {
     ...
     */
    async calculateChurnRisk(userId, forceRefresh = false) {
        try {
            // Check cache first
     ...
     */
    async getWinbackRecommendations(userId) {
        try {
     ...
     */
    async getPlayerBehaviorProfile(userId) {
        try {
      ...
      `;
            ...
            const profile = profileResult.rows ? profileResult.rows[0] || {} : profileResult[0] || {};
            return {
                userId,
     ...
     * Apply cold-start heuristics for new players
     */
    calculateColdStartRisk(profile) {
        ...
        return {
            ...
            keyFactors: factors,
            recommendedActions: this.generateWinbackActions(profile, { riskTier }),
     ...
     */
    generateWinbackActions(profile, risk) {
        const actions = [];
    ...
    generateExplanation(riskTier, factors) {
        const explanations = [];
        switch (riskTier) {
            case 'high':
    ...
    getFactorDescription(feature, value) {
        const descriptions = {
    ...
    async getActiveUsers(days) {
        const result = await this.clickhouse.query(`SELECT DISTINCT user_id FROM events WHERE ts >= NOW() - INTERVAL ${days} DAY`, []);
        return result.map(row => row.user_id);
    }
    async storePrediction(prediction) {
        await this.db.query(`INSERT INTO churn_predictions (
...
//# sourceMappingURL=ChurnPredictor.js.map