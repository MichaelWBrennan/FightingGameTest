...
export class ProgressionEngine {
    constructor(db) {
        ...
        Object.defineProperty(this, "xpMultipliers", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
        ...
        Object.defineProperty(this, "performanceMultipliers", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
    ...
    async grantXP(request) {
        try {
            ...
            const adjustedAmount = this.calculateAdjustedXP(request.baseAmount, request.source, request.performanceTier);
            // Start transaction
            ...
            try {
                ...
                await this.updateMasteryRecord(request.userId, request.trackType, request.trackId, newXP, newTotalXP, newLevel);
                // Grant level-up rewards if applicable
                const rewardsGranted = [];
                if (levelUp) {
                    const rewards = await this.grantLevelUpRewards(request.userId, request.trackType, request.trackId, currentState.level + 1, newLevel);
     ...
     */
    async getMasteryState(userId, trackType, trackId) {
        try {
            const result = await this.db.query(`SELECT level, xp, total_xp, prestige_level, last_prestige_xp, created_at, updated_at
         FROM mastery_tracks 
         WHERE user_id = $1 AND track_type = $2 AND track_id = $3`, [userId, trackType, trackId || null]);
     ...
     */
    async triggerPrestige(userId, trackType, trackId) {
        try {
            ...
            try {
                ...
                await this.db.query(`UPDATE mastery_tracks 
           SET level = 1, xp = 0, prestige_level = $1, last_prestige_xp = $2, updated_at = NOW()
           WHERE user_id = $3 AND track_type = $4 AND track_id = $5`, [newPrestigeLevel, currentState.totalXp, userId, trackType, trackId || null]);
                // Grant prestige rewards
                const rewardsGranted = await this.grantPrestigeRewards(userId, trackType, trackId, newPrestigeLevel);
                // Log prestige event
     ...
     */
    async getAllMasteryTracks(userId) {
        try {
            const result = await this.db.query(`SELECT track_type, track_id, level, xp, total_xp, prestige_level, 
         last_prestige_xp, created_at, updated_at
         FROM mastery_tracks WHERE user_id = $1
         ORDER BY track_type, track_id`, [userId]);
     ...
     */
    calculateAdjustedXP(baseAmount, source, performanceTier) {
     ...
     */
    async initializeMasteryTrack(userId, trackType, trackId) {
        try {
            const result = await this.db.query(`INSERT INTO mastery_tracks (
        ...
        RETURNING level, xp, total_xp, prestige_level, last_prestige_xp, created_at, updated_at`, [userId, trackType, trackId || null]);
            const row = result.rows[0];
            return {
                userId,
                trackType,
     ...
     */
    async updateMasteryRecord(userId, trackType, trackId, newXP, newTotalXP, newLevel) {
        await this.db.query(`UPDATE mastery_tracks 
       SET xp = $1, total_xp = $2, level = $3, updated_at = NOW()
       WHERE user_id = $4 AND track_type = $5 AND track_id = $6`, [newXP, newTotalXP, newLevel, userId, trackType, trackId || null]);
    }
    /**
     * Grant level-up rewards
     */
    async grantLevelUpRewards(userId, trackType, trackId, fromLevel, toLevel) {
        const rewards = [];
     ...
     * Grant prestige rewards
     */
    async grantPrestigeRewards(userId, trackType, trackId, prestigeLevel) {
        const rewards = [];
     ...
     */
    async getLevelRewards(trackType, trackId, level) {
    ...
    async grantReward(userId, reward) {
        await this.db.query(`INSERT INTO user_rewards (user_id, reward_id, reward_type, granted_at, source)
       VALUES ($1, $2, $3, NOW(), 'progression')
       ON CONFLICT (user_id, reward_id) DO NOTHING`, [userId, reward.id, reward.type]);
    }
    /**
     * Log XP grant for analytics
     */
    async logXPGrant(request, adjustedAmount, newLevel, previousLevel) {
        await this.db.query(`INSERT INTO xp_grants (
