...
export class SF3CompressionSystem {
    ...
    static compress(data, method = 'lz77') {
        switch (method) {
            case 'lz77':
    ...
    static compressLZ77(data) {
        const input = new Uint8Array(data);
        const output = [];
        // Add header
        output.push(0x4C, 0x5A, 0x37, 0x37); // 'LZ77'
        output.push((input.length >> 0) & 0xFF, (input.length >> 8) & 0xFF, (input.length >> 16) & 0xFF, (input.length >> 24) & 0xFF);
        let pos = 0;
    ...
    static findLongestMatch(data, pos) {
        let bestLength = 0;
        let bestDistance = 0;
